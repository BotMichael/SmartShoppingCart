...
import Global_Var
from log.Logger import EdgeLogger
...
class Edge_Client_Interface:
    ...
    def getReplyFromFog(self):
        ...
        try:
            ...
            self._log.logger.error(self.id + " Error when eval(message)" + str(e))
            self._error_log.error(self.id + " Error when eval(message)" + str(e))
        return message
